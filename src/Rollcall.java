...
//              startMusic.playMusic("video//"+s +".wav", 0, 4.0f);
                ...
                if (vector.isEmpty())
                    g1.drawString("没有任何学生名字",50,180);
                else
                    g1.drawString(nostart, 120, 180);
            }
        }
    };//主体面板
    int green=100;
    int flag=1;
    File file=new File("Name//学生名称.txt");//人名路径
    File[]files=new File[]{file};//文件数组存储一班二班文件
                        ...
                        try {
                            if (mill > stop) {//如果小于暂停时间
                                ...
                                countPanel.repaint();
                                    ...
                                    startMusic.playMusic("video/" + str + ".wav", 0, 4.0f);
    ...
    static JLabel sumLabel=new JLabel();
...
//                FileReader reader = new FileReader(value);//文件字符读取
        ...
        //学生总人数
        sumLabel.setSize(180,40);
        sumLabel.setText("学生总人数"+vector.size());
        sumLabel.setForeground(Color.ORANGE);
        sumLabel.setFont(new Font(fontPath,Font.PLAIN,25));
        sumLabel.setLocation(40,getHeight()-sumLabel.getHeight()-70);
        background.add(sumLabel);
        //黑名单
...
//        sum.setBorder(BorderFactory.createLineBorder(Color.ORANGE));
        ...
        //已抽取进度条面板
        sumPanel.setSize(wid,50);//注意一定要改宽度不然要减到之前的值才会看到效果
...
//        content.setBorder(BorderFactory.createLineBorder(Color.ORANGE));
        ...
        start.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getButton()==MouseEvent.BUTTON1) {
                    if (!vector.isEmpty()) {
                        if (vector.size()==1)//如果集合只有最后一个人了
                        {
                            System.err.println("集合长度现在是"+vector.size());
                            if (startflag == 1) {//startflag为1才可以点
                                ...
                                mill = (int) (Math.random() * 1500);//点了之后mill重新赋值以便更好的抽
                                stop = new Random().nextInt(601);
                                blackflag = 0;
                                blacklist.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
                        ...
                        JOptionPane.showMessageDialog(null,"已经没有学生抽了");
                        countPanel.repaint();
                    }
                }
            }
